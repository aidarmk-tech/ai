...
strict entry now requires zero exhaustion and artificial risk plus 15-second
taker buy ratio >= 0.90 and 5-second ratio >= 0.75. Every entry that would have
passed the 4.3.5 strict gate remains labelled as `TRADE3_SHADOW`, with explicit
`EXHAUSTION_NONZERO`, `ARTIFICIAL_NONZERO`, `TBR15_LOW` and `TBR5_LOW`
blockers for the next audit.
