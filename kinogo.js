...
(function () {
  ...
  function parseCatalog(html) {
    ...
    // Пробуем типичные обёртки карточек на kinogo-подобных сайтах
    var selectors = [
    ...
    // Fallback: любые дивы с ссылкой-картинкой ведущей на BASE
    if (!cards.length) {
  ...
  function parsePlayerUrl(html) {
    ...
    // 2. В inline-скриптах ищем URL плеера
  ...
  function injectMenu() {
    ...
    var check = setInterval(function () {
      ...
      // Lampa рисует меню в .menu__list
      var $menu = $('.menu__list');
