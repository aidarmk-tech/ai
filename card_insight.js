/*!
 * Card Insight for Lampa — v2 (ATV Fixed)
 * Кнопка "Подробнее" на карточке.
 * Исправлена прокрутка пультом на Android TV и стабильность сети.
 */
(function () {
  'use strict';
  ...
  var LANG_NAMES = {
    ...
    ru: 'русский', kk: 'казахский'
  };

  // ====================================================
  // API (Lampa.Network стабильнее Reguest)
  // ====================================================
  ...
  function renderHero(data) {
    ...
    return '<div class="ci-hero" style="background-image:url(\'' + url + '\')"><div class="ci-hero__overlay"></div></div>';
  }

  function renderOverview(data) {
    var html = '';
    if (data.tagline) html += '<div class="ci-tagline">«' + escapeHtml(data.tagline) + '»</div>';
    
    var meta = [];
    if (data.vote_average) meta.push('<span class="ci-meta__item ci-meta__item--rating">★ ' + data.vote_average.toFixed(1) + '</span>');
    var year = (data.release_date || data.first_air_date || '').substring(0, 4);
    if (year) meta.push('<span>' + year + '</span>');
    if (data.runtime) meta.push('<span>' + data.runtime + ' мин</span>');
    
    html += '<div class="ci-meta">' + meta.join('') + '</div>';
    html += '<div class="ci-overview">' + escapeHtml(data.overview || 'Описание отсутствует.') + '</div>';
    return html;
  }
  ...
  function buildFacts(data) {
    ...
    var findCrew = function(job) {
      return crew.filter(function(p) { return p.job === job; }).map(function(p){ return p.name; });
    };
  ...
  function openModal(movie, method) {
    ...
    fetchFullData(method, movie.id, function (err, data) {
      if (err || !data) {
        $body.html('<div class="ci-error">Ошибка TMDB</div>');
        return;
      }

      $body.empty();
      $body.append(renderHero(data));
      ...
      if (data.credits && data.credits.cast.length) {
        ...
        data.credits.cast.slice(0, 10).forEach(function(p) {
          ...
          var $a = $('<div class="ci-actor selector"><div class="ci-actor__photo" '+(img ? 'style="background-image:url('+img+')"' : '')+'>'+(!img ? '<div class="ci-actor__no-photo">👤</div>':'')+'</div><div class="ci-actor__name">'+escapeHtml(p.name)+'</div></div>');
      ...
      // Управление контроллером (для пульта)
      Lampa.Controller.add('card_insight', {
        toggle: function () {
      ...
      Lampa.Controller.toggle('card_insight');
      scroll.update();
    });
  }
  ...
  function initialize() {
    if (window.cardInsightLoaded) return;
    window.cardInsightLoaded = true;
    injectStyles();
    injectButton();
  }
