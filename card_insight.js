...
(function () {
  ...
  var LANG_NAMES = {
    ...
    he:'иврит',el:'греческий',hu:'венгерский',ro:'румынский',ru:'русский'
  };

  function fetchFullData(method, id, callback) {
    var network = new Lampa.Reguest();
    var url = Lampa.TMDB.api(method + '/' + id + '?api_key=' + Lampa.TMDB.key() + '&language=ru&append_to_response=recommendations,similar,credits,keywords');
  ...
  function buildFacts(data) {
    ...
    function findCrew(jobs) {
      var jobSet = {}; jobs.forEach(function(j){jobSet[j]=true;});
      return crew.filter(function(p){return jobSet[p.job];}).map(function(p){return p.name;}).filter(function(n,i,a){return a.indexOf(n)===i;});
    }
    ...
    if (data.budget && data.budget > 1000 && data.revenue && data.revenue > 1000) {
      ...
      var verdict = ratio>=5?'мега-хит':ratio>=2?'окупился ×'+ratio.toFixed(1):ratio>=1?'окупился':'не окупился';
  ...
  function renderHero(data) {
    ...
    return $('<div class="ci-hero" style="background-image:url(\'' + url + '\')">' +
      '<div class="ci-hero__overlay"></div>' +
      (title ? '<div class="ci-hero__title">' + escapeHtml(title) + '</div>' : '') + '</div>');
  }
  ...
  function renderFactsSection(facts) {
    ...
    facts.forEach(function(f) {
      ...
      $list.append('<div class="ci-fact selector" tabindex="0"><div class="ci-fact__icon">'+f.icon+'</div><div class="ci-fact__text">'+label+escapeHtml(f.text)+'</div></div>');
    });
    $sec.append($list);
    return $sec;
  }
  ...
  function renderCastSection(credits) {
    ...
    cast.forEach(function(person) {
      ...
      var $card = $('<div class="ci-actor selector" tabindex="0"><div class="ci-actor__photo"'+(photo?' style="background-image:url(\''+photo+'\')"':'')+'>'+(!photo?'<div class="ci-actor__no-photo">👤</div>':'')+'</div><div class="ci-actor__name">'+escapeHtml(person.name)+'</div>'+(person.character?'<div class="ci-actor__role">'+escapeHtml(person.character)+'</div>':'<div class="ci-actor__role">&nbsp;</div>')+'</div>');
  ...
  function renderCollectionSection(colData, currentId) {
    ...
    var parts = colData.parts.slice().sort(function(a,b){ var da=a.release_date||'',db=b.release_date||''; return da<db?-1:da>db?1:0; });
    var $sec = $('<div class="ci-section"></div>');
    $sec.append('<div class="ci-section-title"><span class="ci-section-title__bar"></span>'+(colData.name?escapeHtml(colData.name):'Коллекция')+'</div>');
    var $grid = $('<div class="ci-grid"></div>');
    parts.forEach(function(item) {
      ...
      var title = item.title||item.name||'Без названия';
      var rating = item.vote_average&&item.vote_average>0?Number(item.vote_average).toFixed(1):'';
      var $card = $('<div class="ci-card'+(isCurrent?' ci-card--current':'')+' selector" tabindex="0"><div class="ci-card__poster"'+(posterUrl?' style="background-image:url(\''+posterUrl+'\')"':'')+'>'+(!posterUrl?'<div class="ci-card__no-poster">🎬</div>':'')+(rating?'<div class="ci-card__rating">★ '+rating+'</div>':'')+(isCurrent?'<div class="ci-card__current-badge">▶ Сейчас</div>':'')+'</div><div class="ci-card__info"><div class="ci-card__title">'+escapeHtml(title)+'</div>'+(year?'<div class="ci-card__year">'+year+'</div>':'')+'</div></div>');
      if (!isCurrent) {
        $card.on('hover:enter', function() {
          Lampa.Activity.push({ url:'', component:'full', id:item.id, method:'movie', card:item, source:'tmdb' });
        });
      }
      $grid.append($card);
    });
  ...
  function fetchAITrivia(title, year, genres, callback) {
    ...
    xhr.send(JSON.stringify({ model:model, messages:[{role:'user',content:prompt}], max_tokens:900, temperature:0.75 }));
  }
  ...
  function renderWikiTriviaSection(facts) {
    ...
    facts.forEach(function(f) { $list.append('<div class="ci-trivia__item selector" tabindex="0">' + escapeHtml(f) + '</div>'); });
    $sec.append($list);
    return $sec;
  }
  ...
  function CardInsightActivity(object) {
    ...
    var loaded = false, contentReady = false, lastFocused = null;
    var self = this;
    ...
    this.build = function(data) {
      ...
      if (data.belongs_to_collection && data.belongs_to_collection.id) {
        ...
        var colUrl = Lampa.TMDB.api('collection/'+data.belongs_to_collection.id+'?api_key='+Lampa.TMDB.key()+'&language=ru');
        colNet.silent(colUrl, function(colData) {
          if (!$page) return;
          var $col = renderCollectionSection(colData, data.id);
          if ($col) $colPlaceholder.replaceWith($col); else $colPlaceholder.remove();
      ...
      fetchAITrivia(aiTitle, aiYear, aiGenres, function(err, triviaFacts) {
        ...
        if ($trivia) $triviaPlaceholder.replaceWith($trivia); else $triviaPlaceholder.remove();
        try { Lampa.Controller.collectionSet($root); } catch(e) {}
      });

      setTimeout(function() { if (!$root) return; try { Lampa.Controller.toggle('content'); } catch(e) {} }, 150);
    };
    ...
    function scrollToEl(el) {
      ...
      raf(function() {
        ...
        var rootEl = $root[0], viewH = rootEl.clientHeight;
        if (viewH > 0) {
          var elTop = 0, node = el;
          while (node && node !== rootEl) { elTop += node.offsetTop; node = node.offsetParent; }
          var elBottom = elTop + el.offsetHeight, margin = 80;
          if (elTop - margin < rootEl.scrollTop) rootEl.scrollTop = Math.max(0, elTop - margin);
          else if (elBottom + margin > rootEl.scrollTop + viewH) rootEl.scrollTop = elBottom + margin - viewH;
        } else {
          try { el.scrollIntoView({ behavior:'auto', block:'nearest' }); } catch(e) { try { el.scrollIntoView(false); } catch(er) {} }
        }
      });
    }
    ...
    $root.on('hover:focus', '.selector', function() { lastFocused = this; scrollToEl(this); });

    this.start = function() {
      var back = this.back;
      function applyHeight() {
        if (!$root || !$root[0]) return;
        var h = window.innerHeight || document.documentElement.clientHeight || screen.availHeight || screen.height || 0;
        if (h > 100) $root[0].style.height = h + 'px';
      }
      applyHeight(); setTimeout(applyHeight, 300);
      Lampa.Controller.add('content', {
        toggle: function() {
          Lampa.Controller.collectionSet($root);
          if (contentReady) {
            var target = (lastFocused && $root[0] && $.contains($root[0], lastFocused)) ? lastFocused : $root.find('.selector').first()[0];
            if (target) { Lampa.Controller.collectionFocus(target, $root); scrollToEl(target); }
          }
        },
        update: function() { Lampa.Controller.collectionSet($root); },
        left: function() {
          if (typeof Navigator!=='undefined' && Navigator.canmove && Navigator.canmove('left')) { Navigator.move('left'); setTimeout(scrollToFocus,50); }
          else { Lampa.Controller.toggle('menu'); }
        },
        right: function() { if (typeof Navigator!=='undefined' && Navigator.move) { Navigator.move('right'); setTimeout(scrollToFocus,50); } },
        up: function() {
          if (typeof Navigator!=='undefined' && Navigator.canmove && Navigator.canmove('up')) { Navigator.move('up'); setTimeout(scrollToFocus,50); }
          else { Lampa.Controller.toggle('head'); }
        },
        down: function() { if (typeof Navigator!=='undefined' && Navigator.move) { Navigator.move('down'); setTimeout(scrollToFocus,50); } },
        back: back
      });
  ...
  function registerComponent() { Lampa.Component.add(COMPONENT_NAME, CardInsightActivity); }

  function appendButton($buttons, movie, method) {
    if (!$buttons || !$buttons.length) return;
    if ($buttons.find('.view--ci').length) return;
    var svgIcon = '<svg width="36" height="36" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="9" stroke="#ffa726" stroke-width="2"/><path d="M12 8v4M12 16h.01" stroke="#ffa726" stroke-width="2" stroke-linecap="round"/></svg>';
    var $btn = $('<div class="full-start__button selector view--ci">' + svgIcon + '<span>Подробнее</span></div>');
    $btn.on('hover:enter', function() {
      Lampa.Activity.push({ url:'', title:'Подробнее: '+(movie.title||movie.name||''), component:COMPONENT_NAME, id:movie.id, method:method, source:'tmdb' });
    });
    $buttons.append($btn);
  }
