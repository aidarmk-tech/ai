...
class DatasetExporter @Inject constructor(
    ...
    private val experimentMarker: ExperimentVersionMarker,
    private val json: Json
) {
    suspend fun exportCsv(): File = withContext(Dispatchers.IO) {
        val snaps = snapshotDao.all()
        val experimentStartedAt = experimentMarker.startedAt()
        val experimentVersion = experimentMarker.version()
        ...
        for (s in snaps) {
            ...
            val exportSnapshot = if (
                experimentStartedAt > 0L &&
                experimentVersion.isNotBlank() &&
                s.snapshotTime >= experimentStartedAt
            ) {
                s.copy(algorithmVersion = experimentVersion)
            } else {
                s
            }
