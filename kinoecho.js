/*!
 * КиноЭхо (KinoEcho) for Lampa
 * Кинематографический лабиринт: три эхо-цепочки, карта связей, режим настроения.
 * Без AI, без API-ключей — только TMDB.
 */
(function () {
  'use strict';
  ...
  function chainAuthor(data, excludeIds, callback) {
    ...
    // Слабая связь: сценарист (или режиссёр если сценариста нет)
  ...
  function buildCrewFacts(data) {
    ...
    if (cast.length >= 3)
      facts.push({ icon: '🎭', text: 'Главные роли: ' + cast.slice(0, 4).map(function (a) { return a.name; }).join(', ') + '. Зайди в профиль актёра — там целый лабиринт его работ.' });

    var studios = (data.production_companies || []).slice(0, 2);
    if (studios.length)
      facts.push({ icon: '🏛️', text: 'Производство: ' + studios.map(function (c) { return c.name; }).join(' × ') + '. Студия сильно влияет на визуальный язык всех своих проектов.' });

    if (kws.length >= 3)
      facts.push({ icon: '🔑', text: 'Ключевые темы: ' + kws.slice(0, 5).map(function (k) { return k.name; }).join(', ') + '. Скрытая цепочка выбирает фильмы именно по этим тегам.' });

    if (data.budget && data.revenue && data.budget > 1e6) {
      var ratio = (data.revenue / data.budget).toFixed(1);
      var verdict = ratio >= 3 ? 'хит' : ratio >= 1 ? 'окупился ×' + ratio : 'не окупился';
      facts.push({ icon: '💰', text: 'Бюджет vs сборы: ' + verdict + ' (×' + ratio + '). Это влияет на то, почему продолжение есть или нет.' });
    }
  ...
  function renderChainSection(icon, title, chainClass, entries, tagClass, tagLabel) {
    ...
    $sec.append(
      '<div class="ke-section-head">' +
        ...
        '<span class="ke-section-count">' + entries.length + ' фильмов</span>' +
      '</div>'
    );
  ...
  function renderFactsSection(facts) {
    ...
    $sec.append(
      '<div class="ke-section-head">' +
        ...
        '<span class="ke-section-title" style="color:#d580ff">Связи и пасхалки</span>' +
      '</div>'
    );
  ...
  function renderMapTab(data, directEntries, hiddenEntries) {
    ...
    if (directEntries && directEntries.length) {
      ...
      $wrap.append('<div class="ke-map-label">🔗 ПРЯМОЕ ЭХО</div>');
      $wrap.append('<div class="ke-map-line"></div>');
      var $n1 = $('<div class="ke-map-nodes"></div>');
      directEntries.slice(0, 6).forEach(function (e) {
        $n1.append(renderCard(e, 'ke-tag--direct', 'эхо'));
      });
      $wrap.append($n1);
    }

    if (hiddenEntries && hiddenEntries.length) {
      $wrap.append('<div class="ke-map-divider">✨ СКРЫТЫЕ СВЯЗИ</div>');
      ...
      hiddenEntries.slice(0, 6).forEach(function (e) {
        $n2.append(renderCard(e, 'ke-tag--hidden', 'скрытое'));
      });
      $wrap.append($n2);
    }

    if (!directEntries.length && !hiddenEntries.length)
      $wrap.append('<div class="ke-empty">Не удалось построить карту связей</div>');
  ...
  function renderMoodTab(data) {
    ...
    $wrap.append('<div class="ke-mood-intro">Выбери настроение — получишь кинематографический трип из лучших фильмов в этой атмосфере.</div>');
    ...
    MOODS.forEach(function (mood) {
      ...
      $btn.on('hover:enter', function () {
        ...
        $results.html('<div class="ke-loading">🌀 Составляю трип...</div>');
        ...
        tmdbGet(path, function (err, d) {
          ...
          if (err || !d || !d.results || !d.results.length) {
            $results.html('<div class="ke-empty">Не нашлось фильмов для этого настроения</div>');
            return;
          }
          ...
          var $strip = $('<div class="ke-strip"></div>');
          entries.forEach(function (e) { $strip.append(renderCard(e, 'ke-tag--mood', 'трип')); });
  ...
  function KinoEchoActivity(object) {
    var $root = $('<div class="ke-root"><div class="ke-page"><div class="ke-loading">🔮 Строим кинолабиринт...</div></div></div>');
    ...
    this.load = function () {
      ...
      tmdbGet(path, function (err, data) {
        ...
        loaded = true;
        if (err || !data) {
          $page.html('<div class="ke-loading">Ошибка загрузки TMDB</div>');
          return;
        }
        self.build(data);
      });
    ...
    this.build = function (data) {
      ...
      $page.append(
        '<div class="ke-hero"' + (bg ? ' style="background-image:url(\'' + bg + '\')"' : '') + '>' +
          ...
          '<div class="ke-hero__content">' +
            '<div class="ke-hero__label">КиноЭхо</div>' +
            ...
            '<div class="ke-hero__meta">' +
              ...
              '<span class="ke-badge ke-badge--purple">🔮 Кинолабиринт</span>' +
            '</div>' +
          '</div>' +
        '</div>'
      );
      ...
      // Tabs
      var tabNames = ['🔗 Цепочки', '🗺️ Карта', '🎭 Настроение'];
      ...
      var $secDirect = renderChainSection('🔗', 'Прямая эхо-цепочка', 'direct', directEntries, 'ke-tag--direct', 'эхо');
      ...
      $t2.html('<div class="ke-loading">🗺️ Строим карту...</div>');
      ...
      chainHidden(data, directIds, function (entries) {
        ...
        var $secH = renderChainSection('✨', 'Скрытая эхо-цепочка', 'hidden', entries, 'ke-tag--hidden', 'скрытое');
  ...
  function appendButton($buttons, movie, method) {
    ...
    var $btn = $(
      '<div class="full-start__button selector view--ke">' +
        svgIcon + '<span>КиноЭхо</span>' +
      '</div>'
    );
    $btn.on('hover:enter', function () {
      Lampa.Activity.push({
        url:       '',
        title:     'КиноЭхо: ' + (movie.title || movie.name || ''),
